# Shiboleth 01_spec_v1 (2026-07-09)

> The v1 specification. Derives from the intent in `04_trial_context_shiboleth_2026-07-08.md` §6f and the stack lock in §6g. The scorecard text in doc 05 is canonical and used verbatim. This spec feeds the PRD and `02_handoff_to_claude_code`. Doctrine: intent-driven, spec-driven, test-driven, reusable, lean, minimum cost, correctness end to end.

## 1. Intent and definition of done

Build Shiboleth, a marketing-compliance monitoring system with a fixed, opinionated architecture, user flow, and UI, that is **product-agnostic and scorecard-agnostic**: the product under scan, its properties, the scorecard, and the library are runtime data, never code. V1 is done when ONE scan of Intuit TurboTax runs end to end, correctly:
...
- Tracing: `LANGSMITH_TRACING=true`, project `shiboleth-v1`.
...
Day 2 (from the locked backlog): screenshots + pins, clustering polish, Missing UI, audit accordion, insights, daily cron, auto-verify lifecycle, Shiboleth MCP server, library audit, PDF export.
