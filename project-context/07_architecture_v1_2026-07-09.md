# 07_architecture_v1 (2026-07-09)

> The HOW document. Scope: exactly the five things no other document answers: runtime choreography, the pause/resume decision, frontend architecture, concrete scaffolding, and cross-cutting payload contracts. Precedence: `06_prd` defines WHY, `01_spec` defines WHAT, this document defines HOW. Where this conflicts with the spec, the spec wins and the conflict is fixed here. Everything not specified here is Claude Code's to decide idiomatically (and log in `code/CLAUDE.md` if notable).
